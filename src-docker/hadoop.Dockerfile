...
# Copy Hadoop tarball
COPY build/downloads/hadoop-3.4.0.tar.gz /tmp/
...
COPY build/config/core-site.xml $HADOOP_HOME/etc/hadoop/
COPY build/config/hdfs-site.xml $HADOOP_HOME/etc/hadoop/
COPY build/config/hadoop-env.sh $HADOOP_HOME/etc/hadoop/
...
# Create sparkuser and set permissions
# RUN useradd -m sparkuser
# RUN chown -R sparkuser:sparkuser /home/hadoop
